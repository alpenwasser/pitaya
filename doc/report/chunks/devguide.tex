...
\noindent\begin{minipage}[t]{0.33\textwidth}
    \noindent\dirtree{%
        ...
        .3 report/.
        .3 verification/.
        .3 vivado-install/\vspace{5.25ex}.
        .2 env/\vspace{5.5ex}.
        .2 firmware/.
        .3 arm/.
        .4 patches/.
...
\emph{Note:} The  following represents  code which  has to  be entered  on the
command line:
\begin{commandshell}
    enter commands here
\end{commandshell}
...
First  we  copy  the  repository  to  a  \textbf{non-shared}  folder  on  the  guest
...
some of the configuration  parameters for calling \code{cliDispatcher.m} which
...
set of  specifications, prefixed with \code{par}  (e.g. \code{pardecFIR}). The
...
command-line mode,  make sure  \emph{not} to start  it with  the \code{-nojvm}
...
property data. \code{coefData} contains the  filter coefficient files for each
filter in the Vivado-compliant \code{.coe} format. This allows a direct import
...
\end{tcolorbox}
\vspace{2ex}
\noindent To  execute \code{cliDispatcher.m}, Matlab's  command-line interface
can be used:
\vspace{2ex}
\begin{tcolorbox}[
    skin=alpenlisting,
...
libuWebSockets~\cite{uws:github} and a header file called \code{json.hpp} from
...
\code{UPGRADE} request. Currently  it simply  serves the  files from  the file
...
the \code{pre} and \code{suf} conditions which have been set.  The server does
not automatically send the recorded  frame; it has to be requested separately.
The argument \code{forceTrigger} always has to be set to \code{true}. Example
...
currently recording  the frame will  be sent immediately.   The \code{channel}
...
\begin{tcolorbox}[
    ...
    title={
        \refstepcounter{listing}
        \textbf{Listing \thelisting:} Reading the Currently Stored Frame
