...
\clearpage
To make up for the finite window size, which has unpleasant effects (e.g. scattering the spectral lines) on the measured signal's spectrum, the signal can be multiplied with a window-function which should weaken the effect.
This can be done by applying known windows such as a Hanning or a Blackman window as shown in Equation~\ref{eq:gui:window} where $w$ is the window and $x$ is the measured time signal. $c$ is a correction factor to account for the amplitude loss the windowing puts on the signal. This factor is obtained with Equation~\ref{eq:gui:window:correction}.
...
After that the frequency spectrum can be calculated as always, using Equation~\ref{eq:gui:onesidedf}.
