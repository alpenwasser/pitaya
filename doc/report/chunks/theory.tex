...
evident that it is in essence a FIR filter with unitary coefficients.
...
Cascading  integrators and  combs results  in the  frequency responses  in the
...
sampling frequencies as an initial stage  in a chain of filters. The following
stages can then be used to compensate for the distortion introduced by the CIC
filter. For those stages,  FIR filtes are well-suited, since  those stages run
at much  lower sampling rates. The fact  that a FIR filter  performs many more
compuations per  sampling cycle than  a CIC filter  is therefore no  longer as
important  for overall  resource usage; the  FIR  filter has  many more  clock
cycles available for computing each output value than the CIC filter.
...
stages have  unit gain. In  the case  of CIC decimators,  this will  result in
register overflow in the integrator stages. This is without harm as long as
